...
* **Two-way ANOVA**. 
  ...
  * **Example**. In addition to comparing three different designs, we could also compare between using a *male* or *female* voice. We can then divide participants over six groups (3 designs * 2 voices), and use two-way ANOVA to test whether the design and voice have an effect on the perceived attractiveness. We can then also test whether there is an *interaction* between the two variables, to see if specific combinations of design and voice are particularly comforting.
