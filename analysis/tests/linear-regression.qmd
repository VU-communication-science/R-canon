...
The residual tells us how bad our prediction is for each data point.
...
In this tutorial we'll show you how to use regression analysis in several steps:
...
The output of the regression analysis gives us values for the two coefficients: the **(intercept)** and the **age** variable (like $b_0$ and $b_1$ in the regression equation).
...
::: {.callout-note title="Visualizing the regression line" collapse="true"}

To get a better view of what this means in practice, it can be helpful to plot the regression line.
This might also help you better draw the connection between the regression equation and our actual model.
...
We can also plot the data points with `show.data = T` and `jitter = T` (jitter only adds some noice to prevent overlap, which makes it easier to interpret).
...
We do also see that the $R^2$ value is very low (0.009): only 0.9% of the variance in `trust_t1` can be explained by `news_consumption`.
