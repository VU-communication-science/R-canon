...
* The coefficient for `age` tells us how much `trust_t1` changes for every *unit increase* in `age`. Our `age` variable is in years, so the coefficient `r b_age` tells us that for every year of age, trust in journalists increases on average by `r b_age` points.
...
Here we see that the $R^2$ is `r r2`, which means that `r round(r2*100,1)`% of the variance in `trust_t1` can be explained by the variance in `age`, which is huge.
...
The $R^2$ value is very low ($`r r2_1`$): only `r round(r2_1*100,1)`% of the variance in `trust_t1` can be explained by `news_consumption`.
...
But when we include the `age` variable in the model, we see that the effect of `news_consumption` is no longer significant ($b = `r b_nc_2`, p = `r p_nc_2`$).
Also note that the $R^2$ value ($`r r2_2`$) is identical to when we only included the `age` variable (in the simple linear regression).
...
So the regular coefficient of `age` tells us that for every year increase in `age`, `trust_t1` increases by `r b_age_2` points.
How does this compare to the effect of `news_consumption`?
This is an impossible mental exercise, because a 1 unit increase in `age` is not the same as a 1 unit increase in `news_consumption`.
...
For every standard deviation increase in `news_consumption`, `trust_t1` only increases by `r b_nc_1z` standard deviations, and after controlling for `age` only by `r b_nc_2z` standard deviations (which is also no longer significant).
...
So the `political_orientation [left]` coefficient tells us that `trust` is for people in the `left` category is `r b_left` points higher than for people in the `center` category, but this difference is not significant (p = `r p_left`)
The `political_orientation [right]` coefficient tells us that `trust` is for people in the `right` category is `r b_right` points lower than for people in the `center` category, but this difference is also not significant (p = `r p_right`).
