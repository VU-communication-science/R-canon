...
Covariance and correlation are fundamental concepts in statistics.
The covariance of two variables tells us something about the extend to which they **change together**.
If one variable goes up, does the other variable go up as well?
This information is the foundation for many statistical methods, because it enables us to quantify relationships between variables.
For example, it allows us to measure: *how is a person's social media use related to their feelings of loneliness?*
...
* if the correlation between a person's social media use and their feelings of loneliness is positive (`> 0`), it means that people that spend a lot of time on social media tend to feel more lonely.
* If the correlation is negative (`< 0`), it means that people that spend a lot of time on social media tend to feel *less* lonely.
...
n = 30
V1 = rnorm(n, 3, 1)
V2_1_positive = V1 + 1 + rnorm(n, 0, 0.5)
V2_2_negative = -V1 + 9 + rnorm(n, 0, 0.5)
V2_3_weak = 5  + rnorm(n, 0, 1)
