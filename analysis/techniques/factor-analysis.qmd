...
[You might hear about *Principal Component Analysis (PCA)*. While PCA also reduces data into fewer dimensions, it has a different goal (data reduction rather than uncovering latent constructs). In this course, we focus only on **Factor Analysis**.]{style="font-size:70%"}
...
```{r, echo=F}
library(tidyverse) 
setwd("/Users/sarahsramota/Desktop/VU 2025/QNRM/R-canon/data")
d <- read_csv("polcomm_data.csv")
```
...
# Look at the Media Literacy items

d |> select(ml_1:ml_10) |> descr()

```

The output shows the means, standard deviations, and ranges. In our case, all items are measured on a 1–5 Likert scale, with no missing values. The averages hover around 3–4, which makes sense for survey data.
...
-   **`nfactors = 3`** → we want three factors.
...
We won’t go into detail here, since we have a full tutorial on [Scale Construction]{.underline}, but here is how you would do it for the scale arising from Factor 1:
